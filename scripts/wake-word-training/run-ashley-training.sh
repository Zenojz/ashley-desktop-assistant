...
workspace=${ASHLEY_TRAINING_WORKSPACE:-${script_dir}/.work}
...
cp "$script_dir/ashley.yml" "$workspace/ashley.yml"
...
# Resolve Homebrew dynamically; do not bake a machine-specific installation
# prefix into this public workflow.
...
"$python_bin" "$script_dir/generate-ashley-clips.py" \
  --workspace "$workspace" \
  --config "$workspace/ashley.yml"
"$python_bin" openWakeWord/openwakeword/train.py \
  --training_config ashley.yml \
  --generate_clips

# The upstream augment command treats the first output file as a completion
# marker. If decoding or feature extraction was interrupted, that file may be
# a preallocated but incomplete memmap. Require the complete four-file set;
# preserve any partial file for diagnosis and regenerate the set together.
feature_dir="$workspace/output/ashley"
feature_files=(
...
model="$workspace/output/ashley.onnx"
if [[ ! -s "$model" ]]; then
  print -u2 "Training completed without producing $model"
  exit 1
fi
single_file_model="$workspace/output/ashley-single.onnx"
"$python_bin" "$script_dir/consolidate-onnx.py" "$model" "$single_file_model"
cp "$single_file_model" "$project_root/assets/wake-word/models/ashley.onnx"
print "Installed trained model at $project_root/assets/wake-word/models/ashley.onnx"
